...
FD-B = Fonte Desabilitada e Bateria não carregada
NSOL = Não tem Sol
  ...
  while (1) 
  {
    ...
    if(bateria < 8)
    {
      if(LDR > 8.5 && (portc.f0 == 0 || portc.f0 == 1))
      {
         ...
         UART1_Write(13);
         portb.f3 = 0;
         portb.f0 = 0;                              //desliga alimentação auxiliar
      ...
      else if(portc.f0 == 0 && LDR < 8.5)           //Não tem sol e nem bateria
      {
      ...
      else if(portc.f0 == 1 && LDR < 8.5)           //se tiver alimentação auxiliar
      {
         portb.f3 = 0;
         portb.f0 = 1;                              //liga alimentação auxiliar
         portb.f1 = 0;                              //deixa habilitado a bateria
         ...
         UART1_Write(10);
         UART1_Write(13);
         mede_bateria();                            //le valor de tensão da bateria
         delay_ms(5000);
         if(bateria > 11.0)
         {
            portb.f3 = 0;
